...
bool kernels_cache::validate_simple_kernel_execution(kernel::ptr krl, int a, int b, int c) {
    ...
    try {
        ...
        cl::Event ev;
        static cl::CommandQueue queue(ctx, device);
        a = std::max(a, 1);
        b = std::max(b, 1);
        c = std::max(c, 1);
        if (queue.enqueueNDRangeKernel(kernel, cl::NDRange(), cl::NDRange(a, b, c), cl::NDRange(a, b, c), nullptr, &ev) != CL_SUCCESS)
            return false;
