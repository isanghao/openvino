    ...
    std::unordered_map<int, dnnl::memory> get_arguments(fully_connected_inst& instance) const override {
        ...
        if (prim->compressed_weights) {
            ...
            if (is_dyn_quan_input && prim->activation_partial_sum.is_valid()) {
                ...
                args.insert({DNNL_ARG_ATTR_USER_PRECOMP_ZERO_POINTS, act_partial_sum_mem->get_onednn_memory(desc)});
            }
        ...
        if (prim->compressed_weights) {
            ...
            if (is_dyn_quan_input && prim->dynamic_quantized_activation) {
                ...
                if (prim->dynamic_quantized_partial_sum) {
                    ...
                    attr->set_zero_points(DNNL_ARG_ATTR_USER_PRECOMP, grouped, dnnl::memory::dims{1, src_group_size_partial_sum}, act_partial_sum_data_type);
                }
            }
