...
layout scatter_elements_update_inst::calc_output_layout(scatter_elements_update_node const& node) {
    ...
    if (indices_size > static_cast<size_t>(output_shape.sizes()[axis])) {
        CLDNN_ERROR_MESSAGE(node.id(),
            "Undefined behavior ScatterElementsUpdate: indices size must not be larger than the output size along the Axis.");
    }

    if (nonempty_indices_dims + static_cast<size_t>(axis) > updates_number_of_dims) {
        CLDNN_ERROR_MESSAGE(node.id(),
            "Undefined behavior ScatterElementsUpdate: indices dimention must not be larger than the updates[:Axis] dimentional size.");
    }
